...
```json
{
  "instagram": {
    ...
    "notes": "buy-accs.net order #637612"
  },
  "youtube": {
    "login": "email@gmail.com",
    "notes": "экспорт куков из Chrome принца"
  },
  "twitter": {
    "login": "username",
    "notes": "экспорт куков из Chrome принца"
...
### Шаг 1: Запуск браузера (DEPRECATED -- use agent-browser)
```bash
# DEPRECATED: was bash ~/.openclaw/workspace/scripts/chromium-launch.sh start
# Use agent-browser v0.25.3+ instead
```

### Шаг 2: Навигация на страницу логина (DEPRECATED)
```bash
# DEPRECATED: was node ~/.openclaw/workspace/scripts/cdp.js navigate ...
# Use agent-browser navigate API instead
```
...
Деплой на сервер:
```bash
bash skills/media-downloader/scripts/deploy-cookies.sh
```

## YouTube -- обновление куков

YouTube не поддерживает программный логин (CAPTCHA, device verification).
Единственный путь -- экспорт куков из браузера принца.

1. Принц устанавливает расширение Cookie-Editor в Chrome
2. Заходит на youtube.com (залогинен)
3. Открывает Cookie-Editor → Export → Header String
4. Скидывает текст агенту
5. Агент сохраняет в cookies.json:
```json
{
  "youtube": ["<cookie-строка>"]
}
```
6. Деплой: `bash skills/media-downloader/scripts/deploy-cookies.sh`

## Twitter -- обновление куков

Аналогично YouTube:
1. Принц экспортирует из Chrome
...
### Instagram
```bash
ssh root@<your-server-ip> 'curl -s "https://i.instagram.com/api/v1/accounts/current_user/" \
...
2. Ссылка на скачивание приходит на email (<your-email>)
