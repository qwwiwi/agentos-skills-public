...
On the first agent run, OpenClaw bootstraps the workspace (default `~/.claude-lab/silvana`):
...
When the Gateway runs on another machine, edit workspace files on the gateway host (for example, `user@gateway-host:~/.claude-lab/silvana`).
