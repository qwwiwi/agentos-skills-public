...
-   **Tailoring lives outside the repo:** `~/.claude-lab/silvana` (workspace) + `~/.openclaw/openclaw.json` (config).
...
-   **Workspace:** `~/.claude-lab/silvana` (skills, prompts, memories; make it a private git repo)
...
-   Keep `~/.claude-lab/silvana` and `~/.openclaw/` as “your stuff”; don’t put personal prompts/config into the `openclaw` repo.
