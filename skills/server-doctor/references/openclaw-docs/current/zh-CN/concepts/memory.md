...
[跳转到主要内容](https://docs.openclaw.ai/zh-CN/concepts/memory)
...
-   [记忆文件（Markdown）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [何时写入记忆](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [自动记忆刷新（压缩前触发）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [向量记忆搜索](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [额外记忆路径](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [Gemini 嵌入（原生）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [记忆工具的工作原理](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [索引内容（及时机）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [混合搜索（BM25 + 向量）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [为什么使用混合搜索？](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [我们如何合并结果（当前设计）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [嵌入缓存](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [会话记忆搜索（实验性）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [SQLite 向量加速（sqlite-vec）](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [本地嵌入自动下载](https://docs.openclaw.ai/zh-CN/concepts/memory)
-   [自定义 OpenAI 兼容端点示例](https://docs.openclaw.ai/zh-CN/concepts/memory)
...
OpenClaw 记忆是**智能体工作空间中的纯 Markdown 文件**。这些文件是唯一的事实来源；模型只”记住”写入磁盘的内容。 记忆搜索工具由活动的记忆插件提供（默认：`memory-core`）。使用 `plugins.slots.memory = "none"` 禁用记忆插件。
...
这些文件位于工作空间下（`agents.defaults.workspace`，默认 `~/.claude-lab/silvana`）。完整布局参见[智能体工作空间](https://docs.openclaw.ai/concepts/agent-workspace)。
...
当会话**接近自动压缩**时，OpenClaw 会触发一个**静默的智能体回合**，提醒模型在上下文被压缩**之前**写入持久记忆。默认提示明确说明模型_可以回复_，但通常 `NO_REPLY` 是正确的响应，因此用户永远不会看到这个回合。 这由 `agents.defaults.compaction.memoryFlush` 控制：
...
完整的压缩生命周期参见[会话管理 + 压缩](https://docs.openclaw.ai/reference/session-management-compaction)。
...
OpenClaw 可以在 `MEMORY.md` 和 `memory/*.md`（以及你选择加入的任何额外目录或文件）上构建小型向量索引，以便语义查询可以找到相关笔记，即使措辞不同。 默认值：
...
这不是”IR 理论完美”的，但它简单、快速，并且往往能提高真实笔记的召回率/精确率。 如果我们以后想要更复杂的方案，常见的下一步是倒数排名融合（RRF）或在混合之前进行分数归一化（最小/最大或 z 分数）。 配置：
...
OpenClaw 可以在 SQLite 中缓存**块嵌入**，这样重新索引和频繁更新（特别是会话记录）不会重新嵌入未更改的文本。 配置：
...
你可以选择性地索引**会话记录**并通过 `memory_search` 呈现它们。 这由实验性标志控制。
...
当 sqlite-vec 扩展可用时，OpenClaw 将嵌入存储在 SQLite 虚拟表（`vec0`）中，并在数据库中执行向量距离查询。这使搜索保持快速，无需将每个嵌入加载到 JS 中。 配置（可选）：
...
[会话工具](https://docs.openclaw.ai/zh-CN/concepts/session-tool)[压缩](https://docs.openclaw.ai/zh-CN/concepts/compaction)
