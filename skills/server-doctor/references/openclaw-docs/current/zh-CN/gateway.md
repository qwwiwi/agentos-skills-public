<!-- Source: https://docs.openclaw.ai/zh-CN/gateway -->
<!-- Title: Gateway 网关运行手册 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/gateway)
...
-   [Gateway 网关服务运行手册](https://docs.openclaw.ai/zh-CN/gateway)
-   [是什么](https://docs.openclaw.ai/zh-CN/gateway)
-   [如何运行（本地）](https://docs.openclaw.ai/zh-CN/gateway)
-   [远程访问](https://docs.openclaw.ai/zh-CN/gateway)
-   [多个 Gateway 网关（同一主机）](https://docs.openclaw.ai/zh-CN/gateway)
-   [Dev 配置文件（--dev）](https://docs.openclaw.ai/zh-CN/gateway)
-   [协议（运维视角）](https://docs.openclaw.ai/zh-CN/gateway)
-   [方法（初始集）](https://docs.openclaw.ai/zh-CN/gateway)
-   [事件](https://docs.openclaw.ai/zh-CN/gateway)
-   [WebChat 集成](https://docs.openclaw.ai/zh-CN/gateway)
-   [类型和验证](https://docs.openclaw.ai/zh-CN/gateway)
-   [连接快照](https://docs.openclaw.ai/zh-CN/gateway)
-   [错误码（res.error 形状）](https://docs.openclaw.ai/zh-CN/gateway)
-   [保活行为](https://docs.openclaw.ai/zh-CN/gateway)
-   [重放 / 间隙](https://docs.openclaw.ai/zh-CN/gateway)
-   [监管（macOS 示例）](https://docs.openclaw.ai/zh-CN/gateway)
-   [Gateway 网关服务管理（CLI）](https://docs.openclaw.ai/zh-CN/gateway)
-   [监管（systemd 用户单元）](https://docs.openclaw.ai/zh-CN/gateway)
-   [Windows（WSL2）](https://docs.openclaw.ai/zh-CN/gateway)
-   [运维检查](https://docs.openclaw.ai/zh-CN/gateway)
-   [安全保证](https://docs.openclaw.ai/zh-CN/gateway)
-   [CLI 辅助工具](https://docs.openclaw.ai/zh-CN/gateway)
-   [迁移指南](https://docs.openclaw.ai/zh-CN/gateway)
...
-   默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.claude-lab/silvana/canvas` 提供 `http://<gateway-host>:18793/__openclaw__/canvas/`。使用 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
通常不需要：一个 Gateway 网关可以服务多个消息渠道和智能体。仅在需要冗余或严格隔离（例如：救援机器人）时使用多个 Gateway 网关。 如果你隔离状态 + 配置并使用唯一端口，则支持。完整指南：[多个 Gateway 网关](https://docs.openclaw.ai/gateway/multiple-gateways)。 服务名称是配置文件感知的：
...
救援机器人模式：保持第二个 Gateway 网关隔离，使用自己的配置文件、状态目录、工作区和基础端口间隔。完整指南：[救援机器人指南](https://docs.openclaw.ai/gateway/multiple-gateways)。
...
-   当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.claude-lab/silvana-dev`。
...
-   完整文档：[Gateway 网关协议](https://docs.openclaw.ai/gateway/protocol) 和 [Bridge 协议（旧版）](https://docs.openclaw.ai/gateway/bridge-protocol)。
...
另见：[Presence](https://docs.openclaw.ai/concepts/presence) 了解 presence 如何产生/去重以及为什么稳定的 `client.instanceId` 很重要。
...
-   如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 OpenClaw 配置文件服务。 我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](https://docs.openclaw.ai/gateway/multiple-gateways)。
...
-   OpenClaw.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.openclaw.*` 标签仍能干净卸载）的按用户 LaunchAgent。
...
OpenClaw 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们 建议单用户机器使用用户服务（更简单的 env，按用户配置）。 对于多用户或常驻服务器使用**系统服务**（无需 lingering， 共享监管）。 `openclaw gateway install` 写入用户单元。`openclaw doctor` 审计 单元并可以将其更新以匹配当前推荐的默认值。 创建 `~/.config/systemd/user/openclaw-gateway[-<profile>].service`：
...
**替代方案（系统服务）** - 对于常驻或多用户服务器，你可以 安装 systemd **系统**单元而不是用户单元（无需 lingering）。 创建 `/etc/systemd/system/openclaw-gateway[-<profile>].service`（复制上面的单元， 切换 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：
