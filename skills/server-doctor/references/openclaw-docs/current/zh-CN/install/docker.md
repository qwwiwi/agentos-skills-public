<!-- Source: https://docs.openclaw.ai/zh-CN/install/docker -->
<!-- Title: Docker - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/install/docker)
...
-   [Docker（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [Docker 适合我吗？](https://docs.openclaw.ai/zh-CN/install/docker)
-   [要求](https://docs.openclaw.ai/zh-CN/install/docker)
-   [容器化 Gateway 网关（Docker Compose）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [快速开始（推荐）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [手动流程（compose）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [控制 UI 令牌 + 配对（Docker）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [额外挂载（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [持久化整个容器 home（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [安装额外的 apt 包（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [高级用户/功能完整的容器（选择加入）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [权限 + EACCES](https://docs.openclaw.ai/zh-CN/install/docker)
-   [更快的重建（推荐）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [渠道设置（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [OpenAI Codex OAuth（无头 Docker）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [健康检查](https://docs.openclaw.ai/zh-CN/install/docker)
-   [E2E 冒烟测试（Docker）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [QR 导入冒烟测试（Docker）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [注意](https://docs.openclaw.ai/zh-CN/install/docker)
-   [智能体沙箱（主机 Gateway 网关 + Docker 工具）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [它做什么](https://docs.openclaw.ai/zh-CN/install/docker)
-   [每智能体沙箱配置文件（多智能体）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [默认行为](https://docs.openclaw.ai/zh-CN/install/docker)
-   [启用沙箱隔离](https://docs.openclaw.ai/zh-CN/install/docker)
-   [构建默认沙箱镜像](https://docs.openclaw.ai/zh-CN/install/docker)
-   [沙箱通用镜像（可选）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [沙箱浏览器镜像](https://docs.openclaw.ai/zh-CN/install/docker)
-   [自定义沙箱镜像](https://docs.openclaw.ai/zh-CN/install/docker)
-   [工具策略（允许/拒绝）](https://docs.openclaw.ai/zh-CN/install/docker)
-   [清理策略](https://docs.openclaw.ai/zh-CN/install/docker)
-   [安全注意事项](https://docs.openclaw.ai/zh-CN/install/docker)
-   [故障排除](https://docs.openclaw.ai/zh-CN/install/docker)
...
-   **沙箱注意事项**：智能体沙箱隔离也使用 Docker，但它**不需要**完整的 Gateway 网关在 Docker 中运行。参阅[沙箱隔离](https://docs.openclaw.ai/gateway/sandboxing)。
...
沙箱隔离详情：[沙箱隔离](https://docs.openclaw.ai/gateway/sandboxing)
...
在 VPS 上运行？参阅 [Hetzner（Docker VPS）](https://docs.openclaw.ai/install/hetzner)。
...
更多详情：[仪表板](https://docs.openclaw.ai/web/dashboard)，[设备](https://docs.openclaw.ai/cli/devices)。
...
如果你想将额外的主机目录挂载到容器中，在运行 `docker-setup.sh` 之前设置 `OPENCLAW_EXTRA_MOUNTS`。这接受逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将它们应用到 `openclaw-gateway` 和 `openclaw-cli`。 示例：
...
如果你想让 `/home/node` 在容器重建后持久化，通过 `OPENCLAW_HOME_VOLUME` 设置一个命名卷。这会创建一个 Docker 卷并将其挂载到 `/home/node`，同时保持标准的配置/工作区绑定挂载。这里使用命名卷（不是绑定路径）；对于绑定挂载，使用 `OPENCLAW_EXTRA_MOUNTS`。 示例：
...
如果你需要镜像内的系统包（例如构建工具或媒体库），在运行 `docker-setup.sh` 之前设置 `OPENCLAW_DOCKER_APT_PACKAGES`。这会在镜像构建期间安装包，因此即使容器被删除它们也会持久化。 示例：
...
镜像以 `node`（uid 1000）运行。如果你在 `/home/node/.openclaw` 上看到权限错误，确保你的主机绑定挂载由 uid 1000 拥有。 示例（Linux 主机）：
...
使用 CLI 容器配置渠道，然后在需要时重启 Gateway 网关。 WhatsApp（QR）：
...
文档：[WhatsApp](https://docs.openclaw.ai/channels/whatsapp)，[Telegram](https://docs.openclaw.ai/channels/telegram)，[Discord](https://docs.openclaw.ai/channels/discord)
...
深入了解：[沙箱隔离](https://docs.openclaw.ai/gateway/sandboxing)
...
参阅[多智能体沙箱与工具](https://docs.openclaw.ai/tools/multi-agent-sandbox-tools)了解示例、优先级和故障排除。
...
-   `user` 必须是 root 才能运行 `apt-get`（省略 `user` 或设置 `user: "0:0"`）。 当 `setupCommand`（或 docker 配置）更改时，OpenClaw 会自动重建容器，除非容器是**最近使用的**（在约 5 分钟内）。热容器会记录警告，包含确切的 `openclaw sandbox recreate ...` 命令。
...
加固选项位于 `agents.defaults.sandbox.docker` 下：`network`、`user`、`pidsLimit`、`memory`、`memorySwap`、`cpus`、`ulimits`、`seccompProfile`、`apparmorProfile`、`dns`、`extraHosts`。 多智能体：通过 `agents.list[].sandbox.{docker,browser,prune}.*` 按智能体覆盖 `agents.defaults.sandbox.{docker,browser,prune}.*`（当 `agents.defaults.sandbox.scope` / `agents.list[].sandbox.scope` 是 `"shared"` 时忽略）。
...
这使用 `Dockerfile.sandbox-browser` 构建 `openclaw-sandbox-browser:bookworm-slim`。容器运行启用 CDP 的 Chromium 和可选的 noVNC 观察器（通过 Xvfb 有头）。 注意：
...
[安装器内部机制](https://docs.openclaw.ai/zh-CN/install/installer)[Nix](https://docs.openclaw.ai/zh-CN/install/nix)
