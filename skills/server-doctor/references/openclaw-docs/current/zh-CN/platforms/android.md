<!-- Source: https://docs.openclaw.ai/zh-CN/platforms/android -->
<!-- Title: Android 应用 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/platforms/android)
...
-   [Android 应用（节点）](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [支持概览](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [系统控制](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [连接操作手册](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [前置条件](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [1）启动 Gateway 网关](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [2）验证发现（可选）](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [通过 unicast DNS-SD 的 Tailnet（维也纳 ⇄ 伦敦）发现](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [3）从 Android 连接](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [4）批准配对（CLI）](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [5）验证节点已连接](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [6）聊天 + 历史](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [7）Canvas + 摄像头](https://docs.openclaw.ai/zh-CN/platforms/android)
-   [Gateway 网关 Canvas 主机（推荐用于 web 内容）](https://docs.openclaw.ai/zh-CN/platforms/android)
...
-   安装：[入门指南](https://docs.openclaw.ai/start/getting-started) + [配对](https://docs.openclaw.ai/gateway/pairing)。
-   Gateway 网关：[操作手册](https://docs.openclaw.ai/gateway) + [配置](https://docs.openclaw.ai/gateway/configuration)。
    -   协议：[Gateway 网关协议](https://docs.openclaw.ai/gateway/protocol)（节点 + 控制平面）。
...
系统控制（launchd/systemd）位于 Gateway 网关主机上。参见 [Gateway 网关](https://docs.openclaw.ai/gateway)。
...
Android 节点应用 ⇄（mDNS/NSD + WebSocket）⇄ **Gateway 网关** Android 直接连接到 Gateway 网关 WebSocket（默认 `ws://<host>:18789`）并使用 Gateway 网关拥有的配对。
...
更多调试说明：[Bonjour](https://docs.openclaw.ai/gateway/bonjour)。
...
详情和示例 CoreDNS 配置：[Bonjour](https://docs.openclaw.ai/gateway/bonjour)。
...
配对详情：[Gateway 网关配对](https://docs.openclaw.ai/gateway/pairing)。
...
如果你想让节点显示智能体可以在磁盘上编辑的真实 HTML/CSS/JS，请将节点指向 Gateway 网关 canvas 主机。 注意：节点使用 `canvasHost.port`（默认 `18793`）上的独立 canvas 主机。

1.  在 Gateway 网关主机上创建 `~/.claude-lab/silvana/canvas/index.html`。
2.  将节点导航到它（局域网）：
...
Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18793/__openclaw__/canvas/`。 此服务器将实时重载客户端注入 HTML 并在文件更改时重新加载。 A2UI 主机位于 `http://<gateway-host>:18793/__openclaw__/a2ui/`。 Canvas 命令（仅前台）：
...
参见 [Camera 节点](https://docs.openclaw.ai/nodes/camera) 了解参数和 CLI 助手。

[Windows（WSL2）](https://docs.openclaw.ai/zh-CN/platforms/windows)[iOS 应用](https://docs.openclaw.ai/zh-CN/platforms/ios)
