...
-   **个性化设置存放在仓库之外：** `~/.claude-lab/silvana`（工作区）+ `~/.openclaw/openclaw.json`（配置）。
...
-   **工作区：** `~/.claude-lab/silvana`（Skills、提示、记忆；将其设为私有 git 仓库）
...
-   将 `~/.claude-lab/silvana` 和 `~/.openclaw/` 保持为”你的东西”；不要将个人提示/配置放入 `openclaw` 仓库。
