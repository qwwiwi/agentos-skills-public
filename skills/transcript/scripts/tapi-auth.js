...
//   4. API key is saved to ~/.claude/skills/transcript/secrets/transcript-api-key
//      as a plain text file for agent runtime access.
//
...
// Key storage path (plain text file, just the key)
const KEY_REL_PATH = ".claude/skills/transcript/secrets/transcript-api-key";
...
// ============================================================================
// File System Helpers — plain key file
// ============================================================================
...
// Save API key to ~/.claude/skills/transcript/secrets/transcript-api-key
// as a plain text file (just the key, no JSON wrapper).
...
function saveApiKeyToConfigs(key) {
  ...
  const keyFilePath = path.join(home, KEY_REL_PATH);
  const keyFileDir = path.dirname(keyFilePath);

  try {
    ensureDir(keyFileDir);
    const backedUp = backupFile(keyFilePath);
    if (backedUp) {
      filesWritten.push({ path: backedUp, action: "backup", type: "key-backup" });
    }

    fs.writeFileSync(keyFilePath, key.trim() + "\n", { mode: 0o600 });
    filesWritten.push({ path: keyFilePath, action: "written", type: "key-file" });
  } catch (e) {
    warnings.push(`Could not write ${keyFilePath}: ${e.message}`);
  }
...
function cmdHelp() {
  const home = os.homedir();
  const keyPath = path.join(home, KEY_REL_PATH);
  console.log(`
tapi-auth.js v${VERSION} - TranscriptAPI Account Setup
  ...
  email, and once verified, the API key is saved to:
    ${keyPath}
...
USAGE:
  ...
  2. Verify:    node ./scripts/tapi-auth.js verify --token TOKEN --otp CODE
     -> Verifies the code, saves API key. Done.
